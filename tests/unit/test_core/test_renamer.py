...
class TestVideoRenamer:
    ...
    @patch("video_organizer.core.renamer.renamer.TMDBClient")
    def test_enrich_with_tmdb_numeric_3plus_parent_first_then_numeric(
        self, mock_tmdb_client
    ):
        """纯数字 show_name（3位及以上）：优先用父目录名搜索，父目录搜不到时用数字完全匹配兜底"""
        mock_client_instance = MagicMock()
        mock_tmdb_client.return_value = mock_client_instance

        # 父目录名搜索无结果
        mock_client_instance.search_all_pages.return_value = []
        mock_client_instance.search_multi.return_value = {
            "results": [
                {
                    "id": 731,
                    "title": "731",
        ...
        mock_client_instance.get_movie_details.return_value = {
            "title": "731",
            "release_date": "2022-05-01",
            "overview": "",
            "genres": [],
        ...
        mock_client_instance.get_movie_credits.return_value = {"cast": [], "crew": []}
        mock_client_instance.get_external_ids.return_value = {"id": 731, "imdb_id": ""}

        renamer = VideoRenamer("your_tmdb_api_key")
        metadata = {
            "show_name": "国内电影",
            "parent_show_name": "国内电影",
            "season": "7",
            "episode": "31",
            "media_type": "tv",
            "_media_type_confidence": 0.7,
            "cleaned_name": "731",
            "_numeric_3plus_candidate": "731",
        }
        result = renamer._enrich_with_tmdb(metadata)

        assert result.get("tmdb_id") == 731
        assert result.get("media_type") == "movie"
        assert result.get("_numeric_exact_matched") is True
        # 父目录名搜索失败后，数字完全匹配兜底生效
        mock_client_instance.search_multi.assert_called_with("731", language=None)

    @patch("video_organizer.core.renamer.renamer.TMDBClient")
    def test_enrich_with_tmdb_numeric_3plus_no_parent(self, mock_tmdb_client):
        """纯数字 show_name（3位及以上）无父目录：不设置 fallback，主搜索失败后数字兜底"""
        mock_client_instance = MagicMock()
        mock_tmdb_client.return_value = mock_client_instance

        # 主搜索（数字按 guessit 猜测的 tv 类型搜索）无结果
        mock_client_instance.search_all_pages.return_value = []
        mock_client_instance.search_multi.return_value = {
            "results": [
                {
        ...
        assert result.get("media_type") == "movie"
        assert result.get("_numeric_exact_matched") is True
        # 无父目录时不设置 fallback，数字兜底直接生效
        assert "_numeric_fallback" not in result
        mock_client_instance.search_multi.assert_called_with("731", language=None)
